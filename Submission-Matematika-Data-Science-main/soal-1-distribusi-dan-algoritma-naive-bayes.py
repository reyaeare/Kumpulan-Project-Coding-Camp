# -*- coding: utf-8 -*-
"""Copy of soal-1-distribusi-dan-algoritma-naive-bayes.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Gww_XDSGEcSneHcxS_qTSNsyT2dOM8N2
...
def viz_uniform_teoretis(lower_bound, upper_bound, num_samples):
  ...
  data_uniform = generate_rand_uniform(lower_bound, upper_bound, num_samples)
  plt.hist(data_uniform, bins=30, density=True, color='lightgrey', edgecolor='black')
...
def viz_distribusi_gaussian(first_gaussian, second_gaussian, third_gaussian):
  ...
  ax.hist(first_gaussian, bins=32, alpha=0.5, label='first_gaussian')
  ax.hist(second_gaussian, bins=32, alpha=0.5, label='second_gaussian')
  ax.hist(third_gaussian, bins=32, alpha=0.5, label='third_gaussian')
...
def viz_distribusi_binomial(first_binomial, second_binomial, third_binomial):
  ...
  ax.hist(first_binomial, alpha=0.5, label='first_binomial')
  ax.hist(second_binomial, alpha=0.5, label='second_binomial')
  ax.hist(third_binomial, alpha=0.5, label='third_binomial')
  # AKHIRI KODE DI SINI
...
def generate_data_synthetic(breed, features, n_samples, params):
  ...
  for feature in features:
      dist_params = params[breed][feature]

      match feature:
          case 'wingspan_cm' | 'weight_g':
              df[feature] = generate_gaussian(dist_params.miu, dist_params.sigma, n_samples)
          case 'sing_days':
              df[feature] = generate_binomial(dist_params.n_trials, dist_params.probability, n_samples)
          case 'beak_head_ratio':
              df[feature] = generate_rand_uniform(dist_params.lower_bound, dist_params.upper_bound, n_samples)
...
def training_params_estimation(df, features):
    ...
    for breed in sorted(df['breed'].unique()):
        ...
        for feature in df_breed.columns:
            if feature in ['wingspan_cm', 'weight_g']:
                ...
                probability = df_breed[feature].mean() / 30
